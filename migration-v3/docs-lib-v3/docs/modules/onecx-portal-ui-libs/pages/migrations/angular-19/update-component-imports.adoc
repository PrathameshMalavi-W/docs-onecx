...
* `Action`, `BreadcrumbService`, `DataTableColumn`, `ObjectDetailItem`, `RowListGridData`
...
[NOTE]
====
`PortalViewportComponent` still keeps the `<ocx-portal-viewport>` template usage. Only update the import or module source when required. Do not replace the tag with a different template structure.
====
